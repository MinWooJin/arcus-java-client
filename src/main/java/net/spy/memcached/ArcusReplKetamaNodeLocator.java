...
public class ArcusReplKetamaNodeLocator extends SpyObject implements NodeLocator {
  ...
  public void reflectMigratedHash(List<String> mgStateList) {
    ...
    for (String migrations : mgStateList) {
      ...
      for (String s : splitedResponse) {
        if (s == null) {
          return;
        }
      }
      if (splitedResponse.length == 5) {
        reflectMigratedHashMultiMode(splitedResponse);
      } else if (splitedResponse.length == 3) {
        reflectMigratedHashSingleMode(splitedResponse);
      }
    }
  }

  private void reflectMigratedHashMultiMode(String[] strv) {
    ...
    /* FIXME :: error handling for str to integer */
    executeRound = Integer.valueOf(strv[0]);
    existName = strv[1];
    existMoveSlice = Integer.valueOf(strv[2]);
    alterName = strv[3];
    alterMoveSlice = Integer.valueOf(strv[4]);
    ...
    if (executeRound > migrationExecutionRound) {
      ...
      reflectMigrationsZNodeMulti(sortedExistNames, sortedExistNames.get(sortedExistNames.size() - 1), 160, null, 0, migrationMode);
    ...
    if (executeRound == migrationExecutionRound) {
       ...
       */
      if (alterMoveSlice != -3) {
        reflectMigrationsZNodeMulti(sortedExistNames, existName, existMoveSlice, alterName, alterMoveSlice, migrationMode);
      }
    }
    lock.unlock();
  }

  private void reflectMigratedHashSingleMode(String[] strv) {
    /* migrations znode : g0^0^120, g0^158^148*/
    int startSlice;
    int endSlice;
    String alterName;
    ...
    /* FIXME :: error handling for str to integer */
    alterName = strv[0];
    startSlice = Integer.valueOf(strv[1]);
    endSlice = Integer.valueOf(strv[2]);

    lock.lock();
    reflectMigrationsZNodeSingle(alterName, startSlice, endSlice, migrationMode);
    lock.unlock();
  }
  ...
  private void reflectAlterHsliceMigrated(int startSlice, int endSlice, List<Long> alterHPointList, MigrationMode reflectMode) {
    /* hash slice migrated */
    Long from = alterHPointList.get(startSlice);
    Long to = alterHPointList.get(endSlice);
  ...
  private void reflectMigrationsZNodeMulti(List<String> existList, String existName, int existMoveSlice,
    ...
    if (reflectMode == MigrationMode.Init) {
      ...
      assert reflectMode == MigrationMode.Leave;
      if (migrationKetamaGroups.isEmpty() || ketamaGroups.isEmpty()) {
        return;
      }
    }
  ...
  private void reflectMigrationsZNodeSingle(String alterName, int startSlice, int endSlice, MigrationMode reflectMode) {
    if (reflectMode == MigrationMode.Init) {
      return;
    } else if (reflectMode == MigrationMode.Join) {
      if (migrationKetamaGroups.isEmpty()) {
    ...
    if (endSlice == config.getNodeRepetitions()) {
      /* recalculate endSlice when migration done */
      endSlice = (startSlice == 0 ? config.getNodeRepetitions()-1 : startSlice-1);
    }

    if (alterName != null && allAlterGroups.containsKey(alterName)) {
      List<Long> alterHPointList = allHashPoints.get(alterName);
      if (alterHPointList != null) {
        reflectAlterHsliceMigrated(startSlice, endSlice, alterHPointList, reflectMode);
      }
    }
  }
  ...
  private boolean checkFoundExist(String groupName) {
    if (allFailedExistGroups.isEmpty()) {
      return !allExistGroups.containsKey(groupName);
    } else {
      return !allExistGroups.containsKey(groupName) && !allFailedExistGroups.containsKey(groupName);
    }
  }
